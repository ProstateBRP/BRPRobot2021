...
function crc64_result = igtlComputeCrc(dataBuffer)
    ...
    if ~isa(dataBuffer, 'uint8')
        error('Input data buffer must be of type uint8.');
    end
    % CRC64 lookup table (same as in OpenIGTLink C code) - MATLAB 2018 compatible
    persistent table;
    if isempty(table)
        % Use hex2dec with proper string formatting for MATLAB 2018 compatibility
        hex_values = {
        ...
        % Convert hex strings to decimal values using sscanf for better compatibility
        table = uint64(zeros(1, length(hex_values)));
        for i = 1:length(hex_values)
            % Use sscanf for hex conversion which is more reliable in older MATLAB versions
            table(i) = uint64(sscanf(hex_values{i}, '%lx'));
        end
    end
    ...
    for i = 1:length(dataBuffer)
        ...
        % Use older MATLAB syntax for bitwise operations
